...
def get_news():
    ...
    cursor = db.cursor()
    #SELECT
    query = "SELECT title, content, news_id, datetime, flag FROM tbl_news_0466 WHERE flag = 1"
...
def get_news_by_id(news_id):
    ...
    cursor = db.cursor()
    #SELECT
    query = "SELECT title, content, news_id, datetime, flag FROM tbl_news_0466 WHERE news_id = ? AND flag = 1"
...
def insert_news(title, content, datetime, flag):
    db = get_db()
    cursor = db.cursor()
    query = "INSERT INTO tbl_news_0466(title, content, datetime, flag) VALUES (?,?,?,?)"
...
def update_news(news_id, title, content, datetime, flag):
    db = get_db()
    cursor = db.cursor()
    query = "UPDATE tbl_news_0466 SET title = ?, content = ?, datetime = ?, flag = ? WHERE news_id = ?"
...
def patch_news(news_id, flag):
    db = get_db()
    cursor = db.cursor()
    query = "UPDATE tbl_news_0466 SET flag = ? WHERE news_id = ?"
...
def delete_news(news_id):
    db = get_db()
    cursor = db.cursor()
    query = "DELETE FROM tbl_news_0466 WHERE news_id = ?"
    cursor.execute(query, [news_id])
    db.commit()
    return True
